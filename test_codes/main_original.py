...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes, kernel_size=(1,1), strides=(1,1), padding='same', name="transpose_conv_1x1_layer4", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                activation=tf.nn.relu)
    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes, kernel_size=(1,1), strides=(1,1), padding='same', name="transpose_conv_1x1_layer3", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                activation=tf.nn.relu)
    
    vgg_layer7_transpose = tf.layers.conv2d_transpose(vgg_layer7_out, num_classes, kernel_size=(3,3), strides=(2,2), padding='same', name="transpose_vgg_layer7", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                activation=tf.nn.relu)
    
    vgg_skip_layer4 = tf.add(vgg_layer7_transpose, conv_1x1_layer4)
    vgg_layer4_transpose = tf.layers.conv2d_transpose(vgg_skip_layer4, num_classes, kernel_size=(3,3), strides=(2,2), padding='same', name="transpose_vgg_layer4", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                activation=tf.nn.relu)

    vgg_skip_layer3 = tf.add(vgg_layer4_transpose, conv_1x1_layer3)
    vgg_layer3_transpose = tf.layers.conv2d_transpose(vgg_skip_layer3, num_classes, kernel_size=(16,16), strides=(8,8), padding='same', name="transpose_vgg_layer3", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                activation=tf.nn.relu)
...
def train_nn(sess, epochs, batch_size, train_op, accuracy_operation, cross_entropy_loss, input_image,
    ...
    get_batches_fn = helper_original.gen_batch_function(os.path.join(data_dir, 'data_road/training'), image_shape)
...
def run():
    ...
    helper_original.maybe_download_pretrained_vgg(data_dir)
    ...
    with tf.Session() as sess:
        ...
        helper_original.save_inference_samples(runs_dir=runs_dir, data_dir=data_dir, sess=sess,image_shape=image_shape,
