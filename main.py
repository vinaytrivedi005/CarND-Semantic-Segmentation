...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    #1 x 1 convolution
    conv_1x1_layer_7 = tf.layers.conv2d(inputs = vgg_layer7_out, filters = num_classes, kernel_size = (1,1), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))
    
    conv_1x1_layer_4 = tf.layers.conv2d(inputs = vgg_layer4_out, filters = num_classes, kernel_size = (1,1), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))
    
    conv_1x1_layer_3 = tf.layers.conv2d(inputs = vgg_layer3_out, filters = num_classes, kernel_size = (1,1), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))

    vgg_layer_7_upsampled = tf.layers.conv2d_transpose(inputs = conv_1x1_layer_7, filters = num_classes, kernel_size = (3,3), strides = (2,2), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))

    vgg_layer_4_skip = tf.add(vgg_layer_7_upsampled, conv_1x1_layer_4)

    vgg_layer_4_upsampled = tf.layers.conv2d_transpose(inputs = vgg_layer_4_skip, filters = num_classes, kernel_size = (3,3), strides = (2,2), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))


    vgg_layer_3_skip = tf.add(vgg_layer_4_upsampled, conv_1x1_layer_3)

    nn_last_layer = tf.layers.conv2d_transpose(inputs = vgg_layer_3_skip, filters = num_classes, kernel_size = (16,16), strides = (8,8), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(scale = 1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for X, y in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict = { input_image: X,
                    correct_label: y,
                    keep_prob: 0.7,
...
def run():
    ...
    with tf.Session() as sess:
        ...
        get_batches_fn = helper.gen_batch_function(data_path, image_shape)
